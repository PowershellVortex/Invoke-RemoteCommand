...
if( !$PsCredMan ) { Add-Type $PsCredmanUtils }
...
function Get-CredPersist{Param(
    [Parameter(Mandatory=$true)]
...
function Get-CMCredential {Param(
		[Parameter(Mandatory=$false)][AllowEmptyString()][ValidateLength(1,32767)][String]$Target,
		[Parameter(Mandatory=$false)][ValidateSet("GENERIC","DOMAIN_PASSWORD","DOMAIN_CERTIFICATE","DOMAIN_VISIBLE_PASSWORD","GENERIC_CERTIFICATE","DOMAIN_EXTENDED","MAXIMUM","MAXIMUM_EX")]
        [String]$CredType = "DOMAIN_PASSWORD"
    )
    if("GENERIC" -ne $CredType -and 337 -lt $Target.Length) #CRED_MAX_DOMAIN_TARGET_NAME_LENGTH
	{
		[Management.ManagementException]$MgmtException = New-Object Management.ManagementException("Target field is longer ($($Target.Length)) than allowed (max 337 characters)")
    ...
    if ( $Target ){
    [PsUtils.CredMan+Credential]$dataCredentials = New-Object PsUtils.CredMan+Credential
    } else {
	[PsUtils.CredMan+Credential[]]$dataCredentials = [Array]::CreateInstance( [PsUtils.CredMan+Credential], 0 )
    }
	[Int]$Results = 0
	try	{
		if ( $Target ){
            $Results = [PsUtils.CredMan]::CredRead( $Target, $(Get-CredType $CredType), [Ref]$dataCredentials )
        } else {
            $Results = [PsUtils.CredMan]::CredEnum( $Target, [Ref]$dataCredentials )
        }
	} catch { return $_ }
	...
	return $dataCredentials | ? { $_.Persist -ne 0 }
}
function New-CMCredential {
<#
.Synopsis
...
#>
	Param(
		...
		[Parameter(Mandatory=$false)][ValidateLength(0,256)][String]$Comment,
		[Parameter(Mandatory=$false)][ValidateSet("GENERIC","DOMAIN_PASSWORD","DOMAIN_CERTIFICATE","DOMAIN_VISIBLE_PASSWORD","GENERIC_CERTIFICATE","DOMAIN_EXTENDED","MAXIMUM","MAXIMUM_EX")]
        [String]$CredType = "DOMAIN_PASSWORD",
		[Parameter(Mandatory=$false)][ValidateSet("SESSION","LOCAL_MACHINE","ENTERPRISE")][String]$CredPersist = "ENTERPRISE"
	)

	if([String]::IsNullOrEmpty($Target)) { $Target = $UserName }
	...
	switch ( $Target -eq $UserName -and ( "CRED_TYPE_DOMAIN_PASSWORD" -eq $CredType -or "CRED_TYPE_DOMAIN_CERTIFICATE" -eq $CredType )) {
		 $true { $Credential.Flags = [PsUtils.CredMan+CRED_FLAGS]::USERNAME_TARGET }
		$false { $Credential.Flags = [PsUtils.CredMan+CRED_FLAGS]::NONE }
	}
	$Credential.Type = Get-CredType $CredType
	$Credential.TargetName = $Target
	$Credential.UserName = $UserName
	$Credential.AttributeCount = 0
	$Credential.Persist = Get-CredPersist $CredPersist
	...
	[Int]$Results = 0
	try { $Results = [PsUtils.CredMan]::CredWrite($Credential) } catch { return $_ }
	...
	if ( $Results -ne 0 ) { return $Results } # = $null }
}
#endregion
